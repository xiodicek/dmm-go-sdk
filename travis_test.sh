...
# for Travis CI
# Don't run this script at local
# see: https://gist.github.com/ToQoz/7fd1fe3d3c224c194eea
...
find ./*  -maxdepth 10 -name '*.go' -print0 | $XARGS -r0 -L1 dirname | uniq | $XARGS -I$XARGS_ARG sh -c "$CMD_COV && ($CMD_CHECKDIR && $CMD_CONCATCOV) || exit 0"
